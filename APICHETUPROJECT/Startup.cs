...
namespace APICHETUPROJECT
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            // Add DBContext for DB connection stblish
            services.AddDbContext<ApplicationDbContext>(options => options.UseSqlServer(Configuration.GetConnectionString("DefaultConnection"))
            );
            // And now add service.AddDbContext then now go to in Readme.txt

            // registor Respository service
            services.AddTransient<IEmployee,EmployeeService >();
            services.AddTransient<IUsers, UserService>();
            // Now go to Controller and ReadMe.txt Point 4
        }
